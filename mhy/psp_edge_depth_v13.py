...
class PSP_EDGE_DEPTH(object):
    ...
    def build(self, mode, config):
        ...
        depth_fusion = KL.Concatenate(axis=3)([depth_c1, depth_c2, depth_c3])
        depth_fusion = BN(name="depth_c123_bn")(depth_fusion)
        depth_fusion = KL.Activation("relu")(depth_fusion)
        depth_fusion = KL.Conv2D(256, (3, 3), padding="same", name="depth_fusion_conv", use_bias=False)(depth_fusion)
        ...
        depth_conv1 = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", name="depth_conv1", use_bias=False)(depth_conv1)
        depth_conv1 = BN(name="depth_conv1_bn")(depth_conv1)
        depth_conv1 = KL.Activation("relu")(depth_conv1)

        depth_feature = KL.Conv2D(256, (3, 3), strides=(2, 2), padding="same", name="depth_feature", use_bias=False)(depth_conv1)
        depth_feature = BN(name="depth_feature_bn")(depth_feature)
        depth_feature = KL.Activation("relu")(depth_feature)

        depth = KL.Conv2D(1, (3, 3), strides=(1, 1), padding="same", name="middle_depth")(depth_conv1)
        depth = Interp([640, 640])(depth)

        # semantic branch
        x = KL.Concatenate(axis=3)([res, depth_feature, edge_feature])
        x = build_pyramid_pooling_module(x, [640, 640], branch="_semantic")

        x = KL.Conv2D(640, (3, 3), strides=(1, 1), padding="same", name="final_conv1", use_bias=False)(x)
        x = BN(name="final_conv1_bn")(x)
        x = KL.Activation('relu')(x)

        x = KL.Conv2D(1, (3, 3), strides=(1, 1), name="final_conv2")(x)
        x = Interp([640, 640])(x)
