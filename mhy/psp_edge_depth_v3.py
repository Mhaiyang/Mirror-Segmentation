...
def depth_loss_graph(input_gt_depth, depth):
    ...
    loss = Kloss.mean_squared_error(y_true=gt_depth, y_pred=depth)
...
def load_image_gt(dataset, config, image_id, augment=False):
    ...
    depth = utils.resize_mask(depth, scale, padding, crop)
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            input_gt_depth = KL.Input(
                shape=[config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1]], name="input_gt_depth", dtype=tf.uint8)

        # Build the backbone layers.
        res, C1, C2 = ResNet(input_image, layers=101)
        ...
        y = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="edge_conv3")(y)
        y = KL.Conv2D(256, (3, 3), strides=(2, 2), padding="same", activation="relu", name="edge_conv4")(y)
        y = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="edge_conv5")(y)
        y = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="edge_conv6")(y)

        edge = KL.Conv2D(1, (3, 3), strides=(1, 1), padding="same", name="middle_edge")(y)
        edge = Interp([640, 640])(edge)
        edge = KL.Activation("sigmoid")(edge)

        # depth branch. 1/2 to 1/8 of input image
        z = KL.Conv2D(256, (3, 3), strides=(2, 2), padding="same", activation="relu", name="depth_conv1")(C1)
        z = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="depth_conv2")(z)
        z = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="depth_conv3")(z)
        z = KL.Conv2D(256, (3, 3), strides=(2, 2), padding="same", activation="relu", name="depth_conv4")(z)
        z = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="depth_conv5")(z)
        z = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="depth_conv6")(z)

        depth = KL.Conv2D(1, (3, 3), strides=(1, 1), padding="same", name="middle_depth")(z)
        depth = Interp([640, 640])(depth)

        # final fusion
        m = KL.Concatenate(axis=3, name="fusion")([x, y, z])
        m = KL.Conv2D(1, (3, 3), strides=(1, 1), name="conv6_mirror")(m)
        m = Interp([640, 640])(m)
