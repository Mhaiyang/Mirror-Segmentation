...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    while True:
        try:
            ...
            # Batch full?
            if b >= batch_size:
                inputs = [batch_images, batch_gt_masks, batch_gt_edge]
                outputs = []
...
def build_pyramid_pooling_module(res, input_shape, branch="_semantic"):
    ...
    interp_block4 = interp_block(res, 4, feature_map_size, input_shape)
    interp_block8 = interp_block(res, 8, feature_map_size, input_shape)
...
class PSP_EDGE(object):
    ...
    def build(self, mode, config):
        ...
        # semantic branch
        psp = build_pyramid_pooling_module(res, [640, 640])
        ...
        x = KL.Conv2D(1, (3, 3), strides=(1, 1), padding="same", name="predict_mask")(x)
        x = Interp([640, 640])(x)
        predict_mask = KL.Activation("sigmoid")(x)

        # edge branch. 1/2 of input image
        y = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", activation="relu", name="edge_conv1")(C1)
        ...
        y = KL.Conv2D(1, (3, 3), strides=(1, 1), padding="same", name="predict_edge")(y)
        y = Interp([640, 640])(y)
        predict_edge = KL.Activation("sigmoid")(y)

        if mode == "training":
            # edge losss
            edge_loss = KL.Lambda(lambda x: edge_loss_graph(*x), name="edge_loss")([input_gt_edge, predict_edge])
            # final loss
            mask_loss = KL.Lambda(lambda x: mask_loss_graph(*x), name="mask_loss")([input_gt_mask, predict_mask])

            # Model
            inputs = [input_image, input_gt_mask, input_gt_edge]
            outputs = [predict_mask, predict_edge, edge_loss, mask_loss]
            model = KM.Model(inputs, outputs, name='PSP_EDGE')
            model.load_weights(self.config.Pretrained_Model_Path, by_name=True)

        else:
            model = KM.Model(input_image, [predict_mask, predict_edge], name='PSP_EDGE')
